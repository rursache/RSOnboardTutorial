...
class AnnotationViewController: SpotlightViewController {
    ...
    func next(_ labelAnimated: Bool) {
        ...
        switch stepIndex {
            ...
            case 3:
                spotlightView.move(Spotlight.RoundedRect(center: CGPoint(x: UIScreen.main.bounds.size.width / 2, y: getStatusBarHeight() + navigationBarHeight / 2), size: CGSize(width: 120, height: 40), cornerRadius: 6), moveType: .disappear)
            case 4:
    ...
    func setupAnnotationViewPosition() {
        ...
        annotationViews.enumerated().forEach { (offset, annotationView) in
            switch offset {
                ...
                case 3:
                    annotationView.center = CGPoint(x: UIScreen.main.bounds.size.width / 2, y: getStatusBarHeight() + navigationBarHeight + 20)
                case 4:
